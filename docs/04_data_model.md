...
| `is_selected` | INTEGER |  | 是否通过 AI 价值筛选，`1` 表示 `is_ai_news = 1 AND ai_relevance_score >= 70 AND score >= 81`。 |
...
- `is_selected = 1`：`is_ai_news = 1 AND ai_relevance_score >= 70 AND score > 80`（整数实现为 `score >= 81`），需要抓取正文并可进入展示候选。
...
4. 如果 `is_ai_news = 1 AND ai_relevance_score >= 70 AND score >= 81`，写入 `is_selected = 1`。
...
9. 当 `pipeline_state = 'fetched' AND is_selected = 1 AND is_ai_news = 1 AND ai_relevance_score >= 70 AND score >= 81 AND (content_full IS NOT NULL OR content_raw IS NOT NULL)` 时触发翻译。
