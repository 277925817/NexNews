...
- HighScoreList / Top 30 Days 必须渲染为一个整体卡片，包含标题和全部排名项；排名项必须是卡片内部列表行，不得是嵌套独立卡片。
...
- UI visual regression 必须验证浅灰底色契约：页面背景为 `#F3F4F6`，主 surface 为白色或近白，深色旧背景 token 不得作为大面积背景出现。
- UI visual regression 必须验证 Top 30 Days 整体卡片契约：HighScoreList 外层为单一卡片 surface，排名项为卡片内部列表行。
...
| `A-snapshot-ACC-STOP-006-layout-visual-contract` | snapshot | ACC-STOP-006 | public_surface | Home layout, NewsCard density, Top 30 Days overall card, skeleton dimensions, ArticleView reading width and light gray visual tokens match the UI spec. |
...
| `A-static-ACC-STOP-010-local-acceptance-failure-preservation-docs` | ACC-STOP-010 | TASK-031 | static | reports/stages/static.json |
| `A-unit-ACC-STOP-001-local-acceptance-failure-preservation` | ACC-STOP-001 | TASK-031 | unit | reports/stages/unit.json |
...
- HighScoreList 必须只渲染最近 30 天 `translated` 合格新闻；当 fixture 中合格新闻不少于 10 条时渲染 10 条，并按 `score DESC, published_at DESC` 排序。
...
- 面向用户展示的 acceptance RSS fixture link 不得使用 `example.com`、`example.org`、`example.net`、`.test`、`.invalid` 或其他占位域名；正文抽取仍必须通过本地 article fixture 映射完成，不得访问真实网页正文。
- Translation fixture 中的成功 `summary_zh` / `content_zh` 不得包含 fixture/mock/模拟/占位等测试占位词，且必须可由结构化断言证明摘要与正文属于同一新闻。
...
- 内容真实性：translated 详情页的摘要和正文不得仅以非空作为通过条件，必须通过最小长度、段落数量、非占位词和摘要/正文关联断言。
- 链接真实性：用户可见原文链接不得使用 reserved/placeholder host，且浏览器 E2E 必须证明 ArticleView 链接目标等于 API `original_url`。
