...
本文档只定义验收事实、通过条件、失败条件、证据模型、运行状态和停止判定。Gate 到命令和报告路径的执行映射由仓库根目录 `harness.md` 维护。
...
| Stop decision report | `schema_ref`、`schema_version`、`STOP_ALLOWED`、`gate_status`、`passed_gates`、`failed_gates`、`blocked_gates`、`unknown_gates`、`generated_from_reports`。 |
...
- Any `UNKNOWN`、`FAIL`、`TASK_BLOCKED`、`WORKFLOW_BLOCKED` or `ENV_BLOCKED` operand evaluates to `false`.
...
```json
{
  ...
  "blocked_gates": [],
  "unknown_gates": [],
  "generated_from_reports": ["reports/acceptance/ACC-STOP-001.json"],
...
- `STOP_ALLOWED` MUST be `true` only when every required gate status is `PASS`.
...
Codex 可以停止但必须标记 `TASK_BLOCKED`、`WORKFLOW_BLOCKED` 或 `ENV_BLOCKED`，当且仅当：
