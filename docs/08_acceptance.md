...
- Required Gates 和自动化测试证据不得依赖真实 RSS、真实网页、真实 LLM、生产数据库、网络时间或当前系统时间。
- 如果任一 Required Gate 只能通过 live dependency 得到证据，该 gate 必须判定为 `FAIL`、`TASK_BLOCKED`、`WORKFLOW_BLOCKED` 或 `ENV_BLOCKED`。
- `strict_mock` 约束自动化 gate/test 输入来源，不允许用 live dependency 替代 deterministic evidence。
- `local_user_acceptance_status` 是独立 stop input：本地人工验收服务必须运行在 `http://127.0.0.1:8010/` 的 live runtime，使用真实 RSS、真实网页正文抓取、真实 LLM scoring、真实 LLM translation 和本地 SQLite 运行库；该 live 证据只能证明人工验收环境与用户体验，不能替代 Required Gates。
...
- Fixture/mock `latest_news` and `top_ranked_news` expose only `translated` items in the primary reading path; live RSS fallback may expose `untranslated` items only when they are labeled `未翻译` and omit Chinese summary/body fields.
...
- 主页面或榜单把 `ready` / `translation_failed` 暴露为普通新闻条目，或把 live RSS fallback 英文内容伪装成 `translated`。
...
- `GET /api/home` 的 fixture/mock `latest_news` 和 `top_ranked_news` 只包含 `translated` 新闻，且每条都包含非空、非占位 `summary_zh`；live RSS fallback `untranslated` 新闻必须省略 `summary_zh` / `content_zh` 并在 UI 显示 `未翻译`。
- `GET /api/home` 对 `latest_news` 支持 `limit`、`cursor` 和 `next_cursor` 分页；有下一页时返回 `next_cursor`，无下一页时省略，连续分页不得重复 item。
- `GET /api/home` 的 `top_ranked_news` 不使用 cursor pagination。
...
- `GET /api/home` 分页重复返回 item，末页仍返回 `next_cursor`，或把 cursor pagination 应用于 `top_ranked_news`。
...
- Fixture/mock Home News Feed 和 HighScoreList 中每个可点击新闻条目都是 `translated`，点击后必须显示非占位中文摘要和可阅读中文正文；live RSS fallback `untranslated` 条目必须显示 `未翻译`，点击后显示 `摘要和正文暂不可用` 及原因说明。
- Home page 向下滚动接近底部时，News Feed 使用 `next_cursor` 自动加载并追加更多 NewsCard。
- Home page 加载更多失败时保留已加载 NewsCard 并提供可恢复的重试路径。
- Home page 到达末页后继续滚动不再发起下一页请求。
- HighScoreList 不因 News Feed 加载更多而拥有独立分页、独立滚动容器或独立刷新。
- 直接打开首个 ready、untranslated 和首个 translation_failed 详情路由后不得出现无解释空阅读页，必须显示 `摘要和正文暂不可用` 及原因说明。
...
- Home page 无法通过向下滚动追加更多 NewsCard，追加后出现重复卡片，失败加载清空已加载列表，或末页后仍持续请求。
- HighScoreList 随 News Feed 加载更多变成独立分页列表或独立滚动区域。
