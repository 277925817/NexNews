...
try:
    from scripts.harness.reporting import write_test_report_payload
    from scripts.harness.reports import relative_report_path
except ModuleNotFoundError:
    from harness.reporting import write_test_report_payload
    from harness.reports import relative_report_path
...
def report_relative_path(path: Path) -> str:
    return relative_report_path(path)


def write_test_report(path: Path, payload: dict[str, Any]) -> None:
    write_test_report_payload(path, payload)
...
def run_task_005_unit(report_dir: Path, task_id: str) -> int:
    ...
    report = test_report(
        ...
        failure_type=None if passed else "contract",
        error_category=None if passed else "validation",
        referenced_files=["backend/app/services/pipeline.py", "fixtures/llm/scoring.json", "tests/pipeline/test_scoring_filtering.py"],
        commands=[f"python3 scripts/run_harness.py --stage unit --task-id {task_id} --report-dir reports"],
    )
    write_test_report(report_destination(report_dir, "unit", task_id), report)
    return 0 if passed else 1
...
def run_task_006_unit(report_dir: Path, task_id: str) -> int:
    ...
    report = test_report(
        ...
        failure_type=None if passed else "contract",
        error_category=None if passed else "validation",
        referenced_files=["backend/app/services/pipeline.py", "tests/pipeline/test_scoring_filtering.py"],
        commands=[f"python3 scripts/run_harness.py --stage unit --task-id {task_id} --report-dir reports"],
    )
    write_test_report(report_destination(report_dir, "unit", task_id), report)
    return 0 if passed else 1
...
def run_task_009_integration(report_dir: Path, task_id: str) -> int:
    ...
    report = test_report(
        ...
        failure_type=None if passed else "contract",
        error_category=None if passed else "validation",
        referenced_files=["backend/app/services/pipeline.py", "backend/app/db.py", "tests/pipeline/test_pipeline_orchestration.py"],
        commands=[f"python3 scripts/run_harness.py --stage integration --task-id {task_id} --report-dir reports"],
    )
    write_test_report(report_destination(report_dir, "integration", task_id), report)
    return 0 if passed else 1
...
def run_static_product_stage(report_dir: Path) -> int:
    ...
    base_checks = {
        ...
        "A-static-ACC-STOP-010-ai-value-doc-sync": len(tasks_read_issues) == 0,
...
def run_task_033_stage(report_dir: Path, task_id: str, stage: str) -> int:
    ...
    report = test_report(
        ...
        referenced_files=[
            ...
            "tests/test_fixture_config.py",
            "tests/test_api_contract.py",
            "tests/pipeline/test_fetch_translation.py",
            "scripts/run_harness.py",
        ],
        commands=[f"python3 scripts/run_harness.py --stage {stage} --task-id {task_id} --report-dir reports"],
    )
...
def run_task_036_stage(report_dir: Path, task_id: str, stage: str) -> int:
    if stage == "unit":
        ...
        assertion_ids = ["A-integration-ACC-STOP-003-ai-value-filtering"]
        visibility = "internal_evidence"
        failure_type = "integration"
    else:
    ...
    report = test_report(
        ...
        referenced_files=[
            ...
            "fixtures/rss/feeds.json",
            "fixtures/llm/scoring.json",
            "tests/pipeline/test_scoring_filtering.py",
            "tests/test_api_contract.py",
...
def run_task_037_stage(report_dir: Path, task_id: str, stage: str) -> int:
    if stage == "unit":
        ...
        assertion_id = "A-unit-ACC-STOP-007-ai-value-rubric"
    else:
        observed = task_037_integration_observations()
        assertion_id = "A-integration-ACC-STOP-003-ai-value-fallback"
    passed = not observed["issues"] and all(observed["checks"].values())
    report = test_report(
        stage=stage,
        status="passed" if passed else "failed",
...
def task_acceptance_coverage_evidence(report_dir: Path) -> dict[str, Any]:
    ...
    if not isinstance(coverage_items, list) or not coverage_items:
        ...
        if failed_coverage_items:
            ...
            issues.extend(
                task_coverage_issue_by_task(
                    coverage_items,
                    status_key="failed",
                    item_filter=lambda item: item.get("status") != "passed",
                )
            )
...
def task_coverage_issue_by_task(
    items: list[Any],
    *,
    status_key: str,
    item_filter: Callable[[dict[str, Any]], bool],
) -> list[str]:
    grouped: dict[str, list[str]] = {}
    for item in items:
        if not isinstance(item, dict) or not item_filter(item):
            continue
        task_id = str(item.get("task_id") or "unknown")
        item_id = str(item.get("id") or "unknown")
        grouped.setdefault(task_id, []).append(item_id)
    return [
        f"task_acceptance_coverage:{task_id}:{status_key}={','.join(sorted(item_ids))}"
        for task_id, item_ids in sorted(grouped.items())
    ]
